...
for k = [1:gbl_kpoints]
    gbl_Htilde(:,:,k) = gbl_Usqrtinv(:,:,k)*(W(:,:,k)'*H(W(:,:,k), gbl_n, k))*gbl_Usqrtinv(:,:,k);
    gbl_tmp1(:,:,k) = H(W(:,:,k)*gbl_Usqrtinv(:,:,k)*diag(gbl_f)*gbl_Usqrtinv(:,:,k), gbl_n, k);
    ...
    gbl_HWUsi(:,:,k) = H(W(:,:,k), gbl_n, k)*gbl_Usqrtinv(:,:,k);
end
